...
class ReadBinaryWatch {
    fun readBinaryWatch(num: Int): List<String> {
        val hours = mutableListOf<Int>(1, 2, 4, 8)
        val minutes = mutableListOf<Int>(1, 2, 4, 8, 16, 32)
        val res = mutableListOf<String>()
        readBinaryWatch(num, res, hours, minutes, 0, 0)
    ...
    fun readBinaryWatch(num: Int, res: MutableList<String>, hours: List<Int>, minutes: List<Int>, currentHours: Int, currentMin: Int): Boolean {
        var num = num
        if (num == 0 && currentHours < 12 &&currentMin<60) {
            if (currentMin >= 10) {
                res.add("$currentHours:$currentMin")
            } else {
                res.add("$currentHours:0$currentMin")
            }
            return true
        }
        if (num > 0) {
            if (hours.isNotEmpty()) {
                for (i in hours) {
                    if (num > 0) {
                        var hours = hours
                        hours -= i
                        readBinaryWatch(num - 1, res, hours, minutes, currentHours + i, currentMin)
                    }
                }
            }
            if (minutes.isNotEmpty()) {
                for (i in minutes) {
                    if (num > 0) {
                        var minutes = minutes
                        minutes -= i
                        readBinaryWatch(num - 1, res, hours, minutes, currentHours, currentMin + i)
                    }
                }
            }
        }
